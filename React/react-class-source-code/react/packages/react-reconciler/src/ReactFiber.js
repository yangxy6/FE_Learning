...
export function createFiberFromTypeAndProps(
  ...
  if (typeof type === 'function') { // 函数组件或者class组件
    if (shouldConstruct(type)) {
      fiberTag = ClassComponent; // 类组件
    }
  } else if (typeof type === 'string') { // div，span等
...
export function createFiberFromElement(
  ...
  const type = element.type; // DOM上type div,class
  const key = element.key;
  const pendingProps = element.props;//新的props
...
export function createFiberFromFragment(
  ...
  // 对于frament组件直接将element作为props传递给createFiber
