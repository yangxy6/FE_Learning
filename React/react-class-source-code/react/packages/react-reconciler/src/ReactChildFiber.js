...
function ChildReconciler(shouldTrackSideEffects) {
  function deleteChild(returnFiber: Fiber, childToDelete: Fiber): void {
    ...
    childToDelete.effectTag = Deletion; //没有执行真正删除的操作,只更新属性，后续commit会执行真正操作
  }
  ...
  function reconcileChildrenArray(
    ...
    // Add all children to a key map for quick lookups. 数组可能有顺序变化
  ...
  function reconcileChildrenIterator(
    ...
    let step = newChildren.next(); //和数组区别，通过判断step.done
    for (
      ;
  ...
  function reconcileSingleElement(
    ...
    // 从当前已有已有所有节点中找到可以复用新节点，并且把所有兄弟节点都删掉
    while (child !== null) {
