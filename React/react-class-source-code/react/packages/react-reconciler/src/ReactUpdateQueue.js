...
function appendUpdateToQueue<State>(
  ...
  if (queue.lastUpdate === null) {
    // Queue is empty
    queue.firstUpdate = queue.firstUpdate = update;
  } else {
    // 放到最后的操作
...
export function enqueueUpdate<State>(fiber: Fiber, update: Update<State>) {
  ...
  const alternate = fiber.alternate; // current???
  let queue1;
  let queue2;
  ...
  if (queue2 === null || queue1 === queue2) {
    ...
    if (queue1.lastUpdate === null || queue2.lastUpdate === null) {
      ...
      // queue1和queue2内部共享状态，改变一个另一个也会指向，但是最后一个需要手动更改
      appendUpdateToQueue(queue1, update);
...
function ensureWorkInProgressQueueIsAClone<State>(
  ...
  if (current !== null) {
    // If the work-in-progress queue is equal to the current queue, //work-in-progress queue 克隆current queue
