...
function updateFunctionComponent(
  ...
  const unmaskedContext = getUnmaskedContext(workInProgress, Component, true); //context
  const context = getMaskedContext(workInProgress, unmaskedContext);//context
  ...
  if (__DEV__) {
    ...
    nextChildren = Component(nextProps, context);// Component就是声明function  Component
  }

  // React DevTools reads this flag. 记录会被更新到flag用于devtools
...
function updateClassComponent(
  ...
  if (instance === null) {
    ...
    shouldUpdate = true;
  } else if (current === null) { // current===null &&instance！==null之前被中断，instance可以被复用
...
function beginWork(
  ...
  switch (workInProgress.tag) {
    ...
    case FunctionComponent: {
      const Component = workInProgress.type; // function
      const unresolvedProps = workInProgress.pendingProps; // 新的props
      const resolvedProps =
        workInProgress.elementType === Component
          ? unresolvedProps
          : resolveDefaultProps(Component, unresolvedProps);// 目前等于unresolvedProps
      return updateFunctionComponent(
        current,
        workInProgress,
        Component,
