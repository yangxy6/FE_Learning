...
import * as DOMRenderer from 'react-reconciler/inline.dom'; // 操作和平台无关的dom操作及节点调和及任务调度
...
function ReactRoot(
  ...
  const root = DOMRenderer.createContainer(container, isConcurrent, hydrate);// 创建fieberRoot
...
function getReactRootElementInContainer(container: any) {
  ...
  // 通过判断container节点下有没有子节点来判断有否需要调和子节点？？
...
function legacyCreateRootFromDOMContainer(
  ...
  // 没有ssr时 shouldHydrateDueToLegacyHeuristic(container)=false
  const shouldHydrate =
    forceHydrate || shouldHydrateDueToLegacyHeuristic(container);
  // First clear any existing content.
  if (!shouldHydrate) {
    // 当forceHydrate为false,不需要合并节点时，将container下所有节点删除
    let warned = false;
    let rootSibling;
 ...
 * @param {*} children
 * @param {*} container
 * @param {*} forceHydrate render时false
 * @param {*} callback
 */
